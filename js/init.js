...
//alert(document.write("start : init "));
(function($){
  $(function(){
    ...
    $("#resourceLinkOrLocal").change(function() {
        ...
        else
        {
            ...
            $("#resourceField").html('<div class="btn">' +
                        ...
                        '<input name="resourceFile[]" type="file" multiple="multiple">' +
                        '</div><div class="file-path-wrapper">'+
...
//alert(window.location+'/'+$(this).attr("action"));
        
        $.ajax({
            url: window.location+'/../'+$(this).attr("action"), //CALL TO PROCESS.PHP
            type: 'POST',
            data: formData,
            async: true,
            cache: false,
...
//alert("out of executing ajax");
    return false;
});

//alert(document.write("end : init.js "));
